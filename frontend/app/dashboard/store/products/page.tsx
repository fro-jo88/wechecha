...
export default function StoreProductsPage() {
    const { user, loading: authLoading } = useProtectedPage(['STORE_MANAGER']);
    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            // If tab is 'pending', we ask for PENDING_APPROVAL status.
            // If tab is 'all', we ask for standard list (which defaults to ACTIVE/APPROVED in controller if no status sent)
            // But we must also strictly filter by store. The backend 'getAllProducts' does this via RBAC if user is not Super Admin.

            let status;
            if (activeTab === 'pending') {
                status = 'PENDING_APPROVAL';
            }
            // For 'all', we typically want approved items. 
            // The controller defaults to ACTIVE/APPROVED if no status provided.
    ...
    return (
        <div className="p-8 bg-gray-50 min-h-screen">
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-3xl font-bold text-gray-800">My Store Products</h1>
                    <p className="text-gray-500 mt-1">Manage inventory and view products for your store</p>
                </div>
